...
public interface MovieRepository extends JpaRepository<Movie, Long> {
    ...
    List<Movie> findAllDurationIn(List<Integer> durations);
    ...
    Optional<Movie> retrieveAllMovieNames(String name);

    //Write a native query that return the list of movies in a specific range of prices
    @Query(value = "SELECT * FROM movie WHERE price BETWEEN ?1 AND ?2", nativeQuery = true)

    List<Movie> retrieveByDurationsInRange(List<Integer> durations);
